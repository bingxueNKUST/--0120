...
def generate(prompt,scale,seed,steps,height,width):
    ...
    pipe = DiffusionPipeline.from_pretrained(model_id,use_auth_token = auth_token, scheduler=scheduler).to(device)
    ...
    image = pipe(prompt,  height=height, width=width, guidance_scale=scale, num_inference_steps=steps ,generator =generator ).images[0]
